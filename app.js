...
const API = {
    async call(action, data = {}) {
        try {
            ...
            // Always use GET for Google Apps Script compatibility
            // For photos, we'll compress them aggressively in the frontend
            const params = new URLSearchParams({
                action: action
            });

            // Add data to params, handling special cases
            ...
            // Check if URL is too long (typically 8000-8192 chars max)
            if (url.length > 7500) {
                console.warn('⚠️ URL is very long, photo may fail. Consider smaller image.');
            }
